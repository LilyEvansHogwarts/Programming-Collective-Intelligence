...
# package for function and parameter
class node:
...
def evolve(pc,popsize,rankfunction,maxgen=500,elite=0.2,mutationrate=0.1,breedingrate=0.4):
    pop = [makerandomtree(pc) for i in range(popsize)]
    ...
    for i in range(maxgen):
        ...
        if scores[0][0] == 0:
            break
        pop = [scores[i][1] for i in range(topelite)]
        while len(pop)<popsize:
            if random()<mutationrate:
                c = randint(0,topelite-1)
                pop.append(mutate(pop[c],pc,mutationrate))
            elif random()<breedingrate:
                c1 = randint(0,topelite-1)
                c2 = randint(0,topelite-1)
                pop.append(crossover(pop[c1],pop[c2],breedingrate,1))
            else:
                c = randint(0,topelite-1)
                pop.append(pop[c])
...
def simplify(tree):
    ...
    if isinstance(newtree,node):
        children = [simplify(c) for c in tree.children]
        newtree.children = children
        if newtree.name == 'if' and isinstance(children[0],constnode):
            children[0] = children[0].evaluate([])
            return newtree.function(children)
        else:
            for c in children:
                if not isinstance(c,constnode):
                    return newtree
            return constnode(newtree.evaluate([]))
    return newtree
