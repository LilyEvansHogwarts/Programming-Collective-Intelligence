...
def milesdistance(a1,a2):
    ...
    latdif = 69.1*(lat2-lat2)
    longdif = 53*(long2-long1)
...
def scaledata(rows):
    ...
    def scaleinput(d):
        return [(d.data[i]-low[i])/(high[i]-low[i]) for i in range(len(d.data))]
...
def getoffset(rows,gamma=10):
    ...
    sum0 = sum([sum([rbf(v1,v2,gamma) for v2 in l0]) for v1 in l0])
    sum1 = sum([sum([rbf(v1,v2,gamma) for v2 in l1]) for v1 in l1])
    return 1.0/(len(l1)**2)*sum1 - 1.0/(len(l0)**2)*sum0

def nlclassify(point,rows,offset,gamma=10):
    l0 = [row.data for row in rows if row.match == 0]
    l1 = [row.data for row in rows if row.match == 1]
    count0 = len(l0)
    count1 = len(l1)
    sum0 = sum([rbf(point,v,gamma) for v in l0])
    sum1 = sum([rbf(point,v,gamma) for v in l1])
